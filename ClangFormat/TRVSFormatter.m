...
- (void)formatActiveFile {
  if ([TRVSXcode hasSelection]) {
    ...
    [self formatRanges:@[ [NSValue valueWithRange:[TRVSXcode wholeRange]] ]
            inDocument:[TRVSXcode sourceCodeDocument]];
  }
}
...
- (void)formatRanges:(NSArray *)ranges
  ...
  NSArray *continuousLineRanges =
      [self continuousLinesRangeForRanges:lineRanges];
  ...
  [continuousLineRanges enumerateObjectsUsingBlock:^(NSValue *rangeValue, NSUInteger idx, BOOL *stop) {
    ...
    [fragment.string writeToURL:tmpFileURL
                     atomically:YES
                       encoding:NSUTF8StringEncoding
                          error:NULL];
    [self formatFilesAtURLs:@[ tmpFileURL ]];
    fragment.formattedString =
