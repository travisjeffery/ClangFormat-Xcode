...
- (instancetype)initWithStyle:(NSString *)style executablePath:(NSString *)executablePath {
    if (self = [self init]) {
        self.style = style;
...
- (void)formatActiveFile {
    [self formatRanges:@[ [NSValue valueWithRange:[TRVSXcode wholeRangeOfTextView]] ] inDocument:[TRVSXcode sourceCodeDocument]];
}
...
- (void)formatSelectedFiles {
    [[TRVSXcode selectedFileNavigableItems] enumerateObjectsUsingBlock:^(IDEFileNavigableItem *fileNavigableItem, NSUInteger idx, BOOL *stop) {
        ...
        if ([document isKindOfClass:NSClassFromString(@"IDESourceCodeDocument")]) {
            ...
            [self formatRanges:@[ [NSValue valueWithRange:NSMakeRange(0, [[sourceCodeDocument textStorage] length])] ] inDocument:sourceCodeDocument];
...
- (void)formatRanges:(NSArray *)ranges inDocument:(IDESourceCodeDocument *)document {
    ...
    NSArray *fragments = [self fragmentsOfContinuousLineRanges:continuousLineRanges
...
- (NSArray *)selectionRangesAfterReplacingFragments:(NSArray *)fragments
    ...
    [fragments enumerateObjectsUsingBlock:^(TRVSCodeFragment *fragment, NSUInteger idx,
                                            BOOL *stop) {
...
- (NSArray *)fragmentsOfContinuousLineRanges:(NSArray *)continuousLineRanges
    ...
    [continuousLineRanges enumerateObjectsUsingBlock:^(NSValue *rangeValue, NSUInteger idx, BOOL *stop) {
        NSRange characterRange = [textStorage characterRangeForLineRange:[rangeValue rangeValue]];
        ...
        [fragment formatWithStyle:self.style usingClangFormatAtLaunchPath:self.executablePath];
